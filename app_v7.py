...
st.caption("Flagship guided workflow for structured improvement analysis.")
...
if source == "sample":
    ...
    if saved_reviews:
        ...
        loaded_review = load_review_state(review_path)
        selected_defaults = loaded_review.get("project", sample_data)
    else:
        st.sidebar.info("No saved review states found yet.")
        selected_defaults = sample_data

st.markdown('<div class="hero-card"><h3 style="margin:0;">Guide the assessment. Review the evidence. Refine the plan.</h3><div class="small-muted" style="color:#dbeafe;">This version uses a step-by-step flow so non-technical users can validate outputs instead of just reading them.</div></div>', unsafe_allow_html=True)
...
if st.session_state.step == 1:
    ...
    with c1:
        st.download_button("Download markdown summary", markdown_output, file_name=f"{st.session_state.project_data['project_name'].replace(' ', '_').lower()}_summary.md", mime="text/markdown", use_container_width=True)
    with c2:
        st.download_button("Download HTML briefing", html_briefing, file_name=f"{st.session_state.project_data['project_name'].replace(' ', '_').lower()}_briefing.html", mime="text/html", use_container_width=True)
    with c3:
