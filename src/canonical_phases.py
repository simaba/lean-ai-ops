...
def build_role_summary(project: ProjectInput, audience: str, mode: str, improvement_actions: List[EvidenceItem], control_plan: List[EvidenceItem]) -> str:
    if audience == "executive":
        return (
            f"Executive summary: {project.project_name} is being structured in {mode} mode with focus on measurable CTQs, targeted improvement actions, and control discipline to reduce recurrence and improve decision quality."
        )
    if audience == "manager":
        return (
            "Manager summary: prioritize ownership clarity, pilot scope control, metric review cadence, and follow-through on the top improvement actions."
        )
    if audience == "quality_lead":
        return (
            "Quality summary: strengthen CTQs, baseline quality signals, defect or rework tracking, and escalation thresholds."
        )
    if audience == "engineer":
        return (
            "Engineer summary: focus on process handoffs, measurable bottlenecks, standard work, and observable thresholds."
        )
    return (
        "PM summary: keep the problem statement tight, align stakeholders, validate CTQs, and drive one pilot improvement with a visible control plan."
    )
